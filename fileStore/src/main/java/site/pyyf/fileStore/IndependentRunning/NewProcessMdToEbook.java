...
public class NewProcessMdToEbook {
     ...
     */
    public static void replace(String filePath, String dst) throws Exception {
        /* ------------------- 将文件名和后缀分离以便后面使用 ----------------- */
                ...
                if (pointIndex != 0 && rightBracketIndex != 0 && (rightBracketIndex - pointIndex < 10)) {
                    ...
                    if (imgSufix.contains(content.substring(pointIndex + 1, rightBracketIndex ))) {
                        ...
                        if (imgFile.exists()) {
                            ...
                            String insertOssPath = "]("+dst+fileNameWithoutSuffix + "/" + imgName+")";
                            ...
                            cutFile(imgFile.getAbsolutePath(), new File(filePath).getParent() + "\\" + "imgs\\" + fileNameWithoutSuffix + "\\" + imgName);
    ...
    private static void cutFile(String oriPath, String dstPath) {
    ...
    public static void main(String[] args) throws Exception {
        String filePath = "F:\\OneDrive\\笔记\\前端_oss.md";
        String fileName = filePath.split("\\\\")[filePath.split("\\\\").length - 1];
        String fileNameWithoutSuffix = fileName.split("\\.")[0];
//        String dst = "https://pyyf.oss-cn-hangzhou.aliyuncs.com/ebook/";
        String dst = "./imgs/";
        replace(filePath, dst);
